...
Gossip.destroy_all
Tag.destroy_all
JoinTableMessageReceiver.destroy_all
...
PrivateMessage.all.each do |privateMessage|
  rand(1..5).times do
    JoinTableMessageReceiver.create(
      private_message_id: privateMessage.id,
      receiver_id: User.find(rand(User.first.id..User.last.id)).id
    )
  end
end
puts "Pour chaque gossip, j'ai rattaché 1 à 5 Tag"
tp JoinTableMessageReceiver.all
